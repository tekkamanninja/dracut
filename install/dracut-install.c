...
static int install_firmware(struct kmod_module *mod)
{
        ...
        kmod_list_foreach(l, list) {
                ...
                /* FIXME: add more firmware paths */
                asprintf(&fwpath, "/lib/firmware/%s", value);
...
static int install_modules(int argc, char **argv)
{
        ...
        char *srcmods = getenv("srcmods");
        int r = EXIT_SUCCESS;
        int i;

        ctx = kmod_new(srcmods, NULL);
        ...
        for (i = 0; i < argc; i++) {
                ...
                if (argv[i][0] == '/') {
                        ...
                        {
                                asprintf(&path2, "%s/kernel/%s", srcmods, &argv[i][1]);
                                asprintf(&path1, "%s/extra/%s", srcmods, &argv[i][1]);
                                asprintf(&path3, "%s/updates/%s", srcmods, &argv[i][1]);
                        ...
                        r = kmod_module_new_from_lookup(ctx, modname, &modlist);
                        if (r < 0) {
                                log_error("1 Failed to lookup alias '%s': %m", modname);
                                continue; //return r;
                        }
                        if (!modlist) {
